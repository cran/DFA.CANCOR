...
DFA <- function(data, groups, variables, plot=TRUE,
  ...
  evals[,1] <- 1:Ndfs
  anovaDFoutput <- matrix(-9999, Ndfs, 6)
  anovaDFoutputBF <- matrix(-9999, Ndfs, 2)
  ...
  for (luper in 1:Ndfs) {
    ...
    anovaDFoutputBF[luper,1] <- as.numeric(suppressMessages(extractBF(anBF)[1]))
    
    anovaDFoutputBF[luper,2] <- 1 / as.numeric(suppressMessages(extractBF(anBF)[1]))
    
    ttestDFoutput[[luper]] <- GROUP.DIFFS(dd, var.equal=FALSE, MCMC=FALSE, verbose=FALSE)			
  ...
  colnames(anovaDFoutput) <- c('Eta-squared','Wilks_Lambda','F','df','df_res','p')
  rownames(anovaDFoutput) <- colnames(coefs_raw)
  
  colnames(anovaDFoutputBF) <- c('BF_h1h0','BF_h0h1')
  rownames(anovaDFoutputBF) <- colnames(coefs_raw)
  ...
  anovaDVoutput <- matrix(-9999, length(variables), 6)
  anovaDVoutputBF <- matrix(-9999, length(variables), 2)
  ...
  for (lupec in 1:length(variables)) {
    ...
    anovaDVoutputBF[lupec,1] <- as.numeric(extractBF(anBF)[1])
    
    anovaDVoutputBF[lupec,2] <- 1 / as.numeric(extractBF(anBF)[1])
    
    ttestDVoutput[[lupec]] <- GROUP.DIFFS(ddd, var.equal=FALSE, MCMC=FALSE, verbose=FALSE)			
  }
  dimnames(anovaDVoutput) <-list(rep("", dim(anovaDVoutput)[1]))
  colnames(anovaDVoutput) <- c('Eta-squared','Wilks_Lambda','F','df','df_res','p')
  rownames(anovaDVoutput) <- variables
  
  colnames(anovaDVoutputBF) <- c('BF_h1h0','BF_h0h1')
  rownames(anovaDVoutputBF) <- variables
  ...
  if (verbose == TRUE) {
    ...
    message('\n\nOne-way ANOVAs & Bayes Factors using the scores on a discriminant function as the DV:\n')
    print(round_boc(anovaDFoutput), print.gap=4)
    message('\n')
    print(round_boc(anovaDFoutputBF), print.gap=5)
    
    message('\n\nOne-way ANOVAs & Bayes Factors on the original DVs') 
    ...
    message('\n')
    print(round_boc(anovaDVoutputBF), print.gap=5)
    
    message('\n\n\nt-tests, effect sizes, & Bayes Factors for group differences on the discriminant functions\n\n')
    ...
    for (lupe in 1:length(ttestDFoutput)) {
      ...
      message("\nGroup comparisons - descriptive statistics:\n")
      print(round_boc(resultsM[1:8], round_non_p = 2), print.gap=3)
      
      message("\nGroup comparisons - significance tests\n")
      print(round_boc(resultsM[c(1,5,9:12)], round_non_p = 2), print.gap=3)
      
      message("\nGroup comparisons - Bayes Factors\n")
      print(round_boc(resultsM[c(1,5,20:21)], round_non_p = 2), print.gap=3)
      
      message("\nGroup comparisons - mean differences, confidence intervals, & effect sizes\n")
      print(round_boc(resultsM[c(1,5,13:19)], round_non_p = 2), print.gap=3); message('\n')
    }
    
    message('\nt-tests, effect sizes, & Bayes Factors for group differences on the original DVs\n')
